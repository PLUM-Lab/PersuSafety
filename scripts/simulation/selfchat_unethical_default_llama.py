...
persuader_model = AutoModelForCausalLM.from_pretrained(model_id, device_map=device_id)


def get_llama_output(raw_out):
    outputs = tokenizer.batch_decode(raw_out, skip_special_tokens=True)[0]
    outputs = outputs.split("assistant\n\n")[-1]
    # pdb.set_trace()
    return outputs

output_pattern = r"\[/INST\](.*?)</s>"
def get_mistral_output(raw_out):
    outputs = tokenizer.batch_decode(raw_out)[0]
    outputs = re.findall(output_pattern, outputs)[-1]
